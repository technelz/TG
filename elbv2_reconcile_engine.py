...
def canonicalize_actions(actions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    clean = [strip_aws_generated_keys(a) for a in actions or []]
    return sorted(clean, key=lambda x: json.dumps(x, sort_keys=True))
...
def audit_load_balancers(source: WorldState, target: WorldState, ctx: MatchContext, policy: Policy) -> List[ResourceAudit]:
    ...
    for src_name, src_lb in sorted(source.load_balancers.items()):
        ...
        if not live:
            audit.action = "CREATE" if policy.create_missing_load_balancers else "MANUAL_REVIEW"
            audit.notes.append("Missing target load balancer. Automatic LB creation is disabled by default.")
            audits.append(audit)
            continue
...
def execute_plan(
    ...
    for audit in lb_audits:
        ...
        try:
            if audit.action == "CREATE":
                log(f"[MANUAL-REVIEW] Missing LB {audit.name}; auto LB creation disabled/conservative.")
                continue
...
def parse_args() -> Args:
    parser = argparse.ArgumentParser(description="Enterprise AWS ELBv2 DR Routing Reconciliation Engine")
...
def main() -> int:
    ...
    try:
        log("\n============================================================")
        log(" Enterprise AWS ELBv2 DR Routing Reconciliation Engine")
        log("============================================================")
